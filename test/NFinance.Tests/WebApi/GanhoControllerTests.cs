...
namespace NFinance.Tests.WebApi
{
    public class GanhoControllerTests
    {
        ...
        [Fact]
        public void GanhoController_ListarGanhos_ComSucesso()
        {
            //Arrange
            var id = Guid.NewGuid();
            var id1 = Guid.NewGuid();
            ...
            listaGanho.Add(ganho);
            listaGanho.Add(ganho1);
            var listarGanhos = new ListarGanhosViewModel.Response(listaGanho);
            _ganhoService.ListarGanhos().Returns(listarGanhos);
            var controller = InicializarGanhoController();

            //Act
            var teste = controller.ListarGanhos();
            var okResult = teste.Result as ObjectResult;
            var listarGanhosViewModel = Assert.IsType<ListarGanhosViewModel.Response>(okResult.Value);

            //Assert
            Assert.NotNull(teste);
            Assert.Equal(StatusCodes.Status200OK, okResult.StatusCode);
            //verificar o ganho
            var ganhoTest = listarGanhosViewModel.Find(g => g.Id == id);
            Assert.Equal(id, ganhoTest.Id);
            Assert.Equal(idCliente, ganhoTest.IdCliente);
            Assert.Equal(nomeGanho, ganhoTest.NomeGanho);
            Assert.Equal(valor, ganhoTest.Valor);
            Assert.True(ganhoTest.Recorrente);
            //verificar o ganho1
            var ganhoTest1 = listarGanhosViewModel.Find(g => g.Id == id1);
            Assert.Equal(id1, ganhoTest1.Id);
            Assert.Equal(idCliente, ganhoTest1.IdCliente);
            Assert.Equal(nomeGanho, ganhoTest1.NomeGanho);
            Assert.Equal(valor, ganhoTest1.Valor);
